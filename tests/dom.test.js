...
test('«Привычки»: своя планка точечно, «Все отмечены», пороги пассивны, кредо', async () => {
  ...
  assert.match(scr.querySelector('.bar-note').textContent, /0\s*из\s*2/);
...
test('тексты ожидания разбора: дата понедельника открытия в обоих состояниях', async () => {
  ...
  assert.ok(textA.includes('Разбор откроется в понедельник, ' + fmtShortKey(addKey(curMonday(), 7))), textA);
  ...
  assert.ok(textB.includes('Разбор откроется в понедельник, ' + fmtShortKey(addKey(curMonday(), 14))), textB);

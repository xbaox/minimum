...
const NEUTRAL = {
  ...
  // блик сцены закрытия дня (задача 28.E/C): полупрозрачное высветление
  // поверх заливки планки, живёт 240 мс и ничего не сообщает само по себе.
  // Порога у него нет по той же причине, что у --accent-weak: это не
  // носитель смысла, а тон поверх другого тона
  sheen: 'блик по заливке планки — высветление, не носитель',
...
const BOUND = [
  ...
  // задача 27, п. 7.5: кружок дня в полосе недели ничем, кроме обводки, не
  // нарисован — она несёт «день без отметки», ровно как пустая ячейка цепи.
  // --line-strong давал здесь 1,45:1 в светлой и 1,84:1 в тёмной
  { re: /^\.hstrip i$/, prop: 'border', what: 'обводка кружка дня в полосе недели' }
];

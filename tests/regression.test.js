...
/* Задача 23, п. 7: дыры покрытия, найденные аудитом QA.
   ...
   рендер (полоса дня «Прогресса», ячейка цепи на пороге, черновик листа
   против черновика формы, таймер границы дня), живут в
...
test('З23/7.2: restoreWiped проводит копию как данные извне — посев не запускается', () => {
  ...
  try {
    ...
    assert.deepEqual(app.store.notes, [], 'и ни одной записи посев не приносит');
